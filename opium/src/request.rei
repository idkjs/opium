...
let to_plain_text: t => Lwt.t(string);
...
let to_json: t => Lwt.t(option(Yojson.Safe.t));
...
let to_json_exn: t => Lwt.t(Yojson.Safe.t);
...
let to_urlencoded: t => Lwt.t(list((string, list(string))));
...
let to_multipart_form_data:
  (
    ~callback: (~name: string, ~filename: string, string) => Lwt.t(unit)=?,
    t
  ) =>
  Lwt.t(option(list((string, string))));
...
let headers: (string, t) => list(string);
...
let add_header_or_replace: ((string, string), t) => t;
...
let add_header_unless_exists: ((string, string), t) => t;
...
let add_headers_or_replace: (list((string, string)), t) => t;
...
let add_headers_unless_exists: (list((string, string)), t) => t;
...
let content_type: t => option(string);
...
let set_authorization: (Auth.Credential.t, t) => t;
...
let cookie: (~signed_with: Cookie.Signer.t=?, string, t) => option(string);
...
let cookies: (~signed_with: Cookie.Signer.t=?, t) => list(Cookie.value);
...
let add_cookie: (~sign_with: Cookie.Signer.t=?, Cookie.value, t) => t;
...
let add_cookie_unless_exists:
  (~sign_with: Cookie.Signer.t=?, Cookie.value, t) => t;
...
let urlencoded: (string, t) => Lwt.t(option(string));
...
let urlencoded_exn: (string, t) => Lwt.t(string);
...
let urlencoded_list: (string, t) => Lwt.t(list(string));
...
let query_list: t => list((string, list(string)));
