...
let m = {
  ...
  let filter = (handler, req) => {
    ...
    let body = Body.copy(req.Request.body);
    Lwt.catch(
      () => handler(req),
      exn => {
        Logs.err(~src=log_src, f => f("%s", Nifty.Exn.to_string(exn)));
        let* res_string = format_error({...req, body}, exn);
        let body = Body.of_string(res_string);
