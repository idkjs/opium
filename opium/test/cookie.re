...
let parse_cookies_of_headers = () => {
  let (c1, c2) =
    switch (Opium.Cookie.cookies_of_headers(headers)) {
    | [c1, c2] => (c1, c2)
    | _ => failwith("Unexpected number of cookies parsed")
    };

  Alcotest.(
    check(pair(string, string), "has cookie", ("yummy_cookie", "choco"), c1)
  );
  Alcotest.(
    check(
      pair(string, string),
...
let find_cookie_in_headers = () => {
  let cookie = Opium.Cookie.cookie_of_headers("tasty_cookie", headers);
  Alcotest.(
    check(
